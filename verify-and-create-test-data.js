...
async function verifyAndCreateTestData() {
  console.log('🔍 Checking existing test conversations...\n');

  // Check if test conversations already exist
  const { data: existingConvs, error: checkError } = await supabase
    .from('conversations')
    .select('id, patient_ref')
  ...
  console.log(`Found ${existingConvs.length} existing test conversations`);

  // Delete old test conversations and their messages
  ...
  console.log('🔧 Creating fresh HIPAA-compliant test conversations...\n');
  ...
  const { data: verifyConvs } = await supabase
    .from('conversations')
    .select('id, patient_ref')
    .like('patient_ref', 'patient-TEST%');

  for (const conv of verifyConvs) {
    const { data: messages } = await supabase
      .from('messages')
      .select('id')
      .eq('conversation_id', conv.id);

    console.log(`   ${conv.patient_ref}: ${messages.length} messages`);
  }
