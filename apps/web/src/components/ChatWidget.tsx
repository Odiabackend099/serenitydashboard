...
export default function ChatWidget({
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (e?.message?.includes('Authentication failed')) {
        errorMessage = 'Authentication error. Please refresh the page and try again, or contact support if the issue persists.';
      ...
      } else if (e?.message?.includes('Service error')) {
        errorMessage = 'Service temporarily unavailable. Please try again in a moment.';
      } else if (e?.message) {
        errorMessage = e.message; // Use the specific error message if it's user-friendly
      }
  ...
  return (
    <div className="chat-widget-container">
      {/* Toggle button - optimized for mobile touch */}
      <button
        className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 bg-healthcare-primary dark:bg-healthcare-accent text-white px-4 py-3 sm:px-5 sm:py-4 rounded-full shadow-2xl hover:shadow-3xl hover:scale-105 active:scale-95 hover:bg-healthcare-accent dark:hover:bg-healthcare-primary transition-all duration-200 z-50 flex items-center gap-2 group touch-manipulation"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      ...
      {/* Panel - PWA optimized with safe areas and responsive design */}
      {open && (
        <div
          id="chat-widget"
          className="fixed inset-x-4 bottom-20 sm:inset-x-auto sm:bottom-24 sm:right-6 sm:w-96 md:w-[28rem] h-[calc(100vh-8rem)] sm:h-auto sm:max-h-[calc(100vh-8rem)] bg-white dark:bg-healthcare-dark-bg-elevated border-0 sm:border border-gray-300 dark:border-gray-700 rounded-2xl shadow-2xl flex flex-col z-40 animate-slideInFromRight overflow-hidden"
          role="dialog"
          aria-label="Chat widget"
          aria-modal="true"
        >
          {/* Header - PWA optimized with proper spacing */}
          <div className="shrink-0 p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-healthcare-primary to-healthcare-accent dark:from-healthcare-dark-bg-secondary dark:to-healthcare-dark-bg-secondary text-white rounded-t-2xl flex items-center justify-between gap-2 shadow-sm">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
              <img src="/logo.png" alt="Serenity logo" className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex-shrink-0 shadow-md" />
              <div className="flex flex-col min-w-0 flex-1">
                <span className="font-semibold text-sm sm:text-base truncate">Serenity Assistant</span>
                <span className="text-[10px] sm:text-xs opacity-90 flex items-center gap-1">
                  {mode === 'text' ? (
                    <>
                      <MessageCircle className="w-3 h-3 flex-shrink-0" />
                      <span className="truncate">Text Chat</span>
                    </>
                  ) : (
                    <>
              ...
              {!supabaseConfigured && (
                <span className="hidden sm:inline text-[10px] bg-red-500 px-2 py-0.5 rounded-full font-medium whitespace-nowrap">DB Error</span>
              )}
            </div>
            {!disableVoice && (
              <button
                className="shrink-0 px-2 sm:px-3 py-1.5 sm:py-2 bg-white dark:bg-gray-700 text-healthcare-primary dark:text-healthcare-accent text-xs sm:text-sm rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 active:scale-95 transition-all flex items-center gap-1 font-medium shadow-md touch-manipulation"
                onClick={toggleMode}
          ...
          {/* Messages - PWA optimized scrollable area */}
          <div
            className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-4 space-y-3 sm:space-y-4 bg-gray-50 dark:bg-healthcare-dark-bg-primary overscroll-contain"
            style={{
          ...
          >
            ...
            {!isLoading && messages.map((m, i) => (
              ...
              >
                <div
                  className={
                    'inline-block rounded-2xl px-3 sm:px-4 py-2 sm:py-3 max-w-[85%] sm:max-w-[80%] shadow-md hover:shadow-lg transition-shadow duration-200 ' +
                    (m.role === 'user'
                      ? 'bg-gradient-to-br from-healthcare-primary to-healthcare-accent dark:from-healthcare-accent dark:to-healthcare-primary text-white rounded-br-sm'
                      : 'bg-white dark:bg-healthcare-dark-bg-secondary text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-bl-sm')
                  }
                >
                  <div className="text-[10px] sm:text-xs opacity-70 mb-1 flex items-center justify-between gap-2">
                    <span className="font-medium">{m.from === 'patient' ? 'You' : m.from === 'ai' ? 'AI' : 'Staff'}</span>
                    {m.timestamp && (
                  ...
                  <div className="whitespace-pre-wrap text-sm sm:text-base leading-relaxed break-words">{m.content}</div>
          ...
          {/* Input area - PWA optimized for mobile keyboards */}
          {mode === 'text' ? (
            <div className="shrink-0 p-3 sm:p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-healthcare-dark-bg-elevated rounded-b-2xl shadow-inner">
            ...
            <div className="shrink-0 p-3 sm:p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-healthcare-dark-bg-elevated rounded-b-2xl shadow-inner">
              ...
              voiceStatus.status === 'error' ? (
                <button
                  className="w-full px-4 py-3 sm:py-3.5 bg-gradient-to-r from-green-600 to-green-700 dark:from-green-700 dark:to-green-800 text-white rounded-xl hover:shadow-lg hover:from-green-700 hover:to-green-800 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 transition-all duration-200 font-medium text-sm sm:text-base flex items-center justify-center gap-2 sm:gap-3 shadow-md touch-manipulation"
          ...
          {/* Footer info - PWA optimized */}
          <div className="shrink-0 px-3 sm:px-4 py-2 bg-gray-50 dark:bg-healthcare-dark-bg-secondary text-xs sm:text-sm text-gray-600 dark:text-gray-400 text-center rounded-b-2xl border-t border-gray-200 dark:border-gray-700">
            {conversationId ? (
