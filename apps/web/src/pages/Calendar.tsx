...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time changes using Supabase functions
    const channel = supabaseFunctions.subscribeToAppointments((payload) => {
  ...
  const loadAppointments = async () => {
    try {
      const response = await supabaseFunctions.getAppointments();
      
      if (response.success && response.data) {
        const mappedEvents: FCEvent[] = response.data.appointments.map((appointment: any) => {
